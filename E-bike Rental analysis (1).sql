SELECT * 
FROM e-bike-rental-data-analysis.E_Bike_Rental.rides
LIMIT 1000;

SELECT * 
FROM e-bike-rental-data-analysis.E_Bike_Rental.stations
LIMIT 1000;

SELECT * 
FROM e-bike-rental-data-analysis.E_Bike_Rental.users
LIMIT 1000;
...
SELECT
   (SELECT COUNT(*) 
      FROM e-bike-rental-data-analysis.E_Bike_Rental.rides) AS  total_rides,
     (SELECT COUNT(*) 
      FROM e-bike-rental-data-analysis.E_Bike_Rental.stations) AS  total_stations,
      (SELECT COUNT(*) 
      FROM e-bike-rental-data-analysis.E_Bike_Rental.users) AS  total_usera;
...
SELECT
 ...
 COUNTIF(start_time IS NULL) AS null_start_date,
 COUNTIF(end_time IS NULL) AS null_end_time

FROM e-bike-rental-data-analysis.E_Bike_Rental.rides;

--- Sammary statistics for the rides table
...
--- Checking for false starts for the rides table due to the minimum distance and duration being zero
...
--- Trying to understand diferrent membership
...
FROM e-bike-rental-data-analysis.E_Bike_Rental.rides AS r
JOIN e-bike-rental-data-analysis.E_Bike_Rental.users AS u
  ON r.user_id = u.user_id

GROUP BY u.membership_level 
ORDER BY total_rides DESC;

--- Checking the peek hours
...
--- Checking for the top 10 popular stations
...
FROM e-bike-rental-data-analysis.E_Bike_Rental.rides AS r
JOIN e-bike-rental-data-analysis.E_Bike_Rental.stations AS s
...
FROM e-bike-rental-data-analysis.E_Bike_Rental.stations AS s
